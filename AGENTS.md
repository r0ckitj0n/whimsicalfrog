...
This file is the single consolidated rule source for agent behavior in this repo.
Merged on 2026-02-19 from: `AGENTS.md`, `.cursorrules`, `.agent/workflows/toast-notifications.md`.
Legacy source files were archived to `backups/guidelines-archive/2026-02-19/`.

## Active Ruleset (Consolidated)
...
### Duplicate Filename Rule (`" 2"` Suffix)
- **Default Action:** Any file matching `* 2*` is treated as an accidental duplicate and must be archived immediately to `backups/orphans-archive/<date>/duplicates/` (preserve original relative path).
- **Exceptions:** Only keep a `" 2"` file if there is explicit business need and it is added to `scripts/orphan_whitelist.json` with a clear reason.
- **Limit:** Keep the number of `" 2"` files at zero whenever possible; one or two exceptions maximum if explicitly justified.
...
## Additional Active Rules Imported from Legacy System Rules
...
## Preserved Legacy Rules (Verbatim from .cursorrules)
...
- **Credentials:** Never store plaintext credentials in repo rules. Use `.env` and local secret stores only.
...
## Database Schema Sync Protocol (Local -> Live)

> **MANDATE:** Any change that adds a table/column/index to local MySQL must be synced to live MySQL immediately using add-only schema sync, without full DB overwrite.

### Required workflow for schema changes
1. Apply and validate schema change locally.
2. Run local hygiene/tests.
3. Run add-only live sync dry run for impacted table(s):
   - `php scripts/db/sync_local_schema_to_live.php --table=<table_name>`
4. Apply live sync for impacted table(s):
   - `php scripts/db/sync_local_schema_to_live.php --table=<table_name> --apply`
5. Verify on live by checking table structure and endpoint behavior.

### Guardrails
- Never use full database restore to push routine schema additions.
- Sync script is add-only:
  - Allowed: `CREATE TABLE` (if missing), `ADD COLUMN`, `ADD INDEX`.
  - Not allowed: drop/rename/overwrite existing columns or destructive schema changes.
- If live DB host credentials are unreachable, stop and surface the exact connection error, then request corrected live DB access details before proceeding.

### Live DB Access Constraint (Whimsical Frog)
- This environment may block direct DB connections from non-server sources.
- When direct local->live DB connectivity fails or is restricted, schema patches MUST be executed server-side via website API endpoints.
- Required server-side method:
  1. Build SQL patch file locally.
  2. Execute via `POST /api/database_maintenance.php?action=restore_database` with multipart `backup_file` and `admin_token`.
  3. Prefer idempotent SQL (information_schema checks + conditional ALTER) so reruns are safe.
- Preferred command for email logs patch:
  - `bash scripts/db/deploy_email_logs_schema_patch.sh`

## Security Baseline (Non-Negotiable)

### Endpoint Security
- Every mutating endpoint MUST require explicit auth (`requireAdmin(true)` for admin writes, `requireLogin()` for user writes) unless it is intentionally public.
- Public-write endpoints (registration/newsletter/contact) MUST use strict payload validation, abuse controls, and clear allowlisted behavior.
- Mutating actions MUST be `POST` (or `PUT`/`DELETE`) only. `GET` MUST be read-only.
- Every endpoint with actions MUST enforce an explicit action allowlist.
- Never keep localhost/dev auth bypass code paths in production endpoint logic.

### Input Validation
- Validate and normalize all external input at endpoint boundaries.
- Enforce identifier patterns (e.g., SKU/order/table IDs) with strict regex allowlists.
- Enforce enum allowlists for status/type/provider fields.
- Clamp numeric bounds and enforce length limits on strings/JSON payloads.
- Reject invalid JSON and unsupported content types with explicit 4xx responses.

### SQL Safety
- Parameterize all untrusted values in SQL.
- Dynamic SQL identifiers (table/column/order direction) are prohibited unless validated against strict allowlists.
- Never concatenate raw request data into SQL, including `ORDER BY`, `LIMIT`, table names, and column names.
- Prefer `information_schema` checks for table existence over string-built `SHOW TABLES LIKE` where applicable.

### File and Path Safety
- For any file operation, enforce `realpath` containment in approved base directories.
- Validate filenames with allowlists and block traversal patterns.
- Restrict upload types by MIME + extension, and set explicit max sizes.
- Store runtime artifacts under `/.local/state/` and never commit them.

### CSRF, Session, and Cookie Policy
- Admin/browser-session mutating endpoints MUST enforce CSRF tokens.
- Session cookies must remain `HttpOnly`, `Secure` (HTTPS), and `SameSite=Lax` or stricter.
- Regenerate session ID after login and privilege elevation.

### CORS and Headers
- Do not use wildcard CORS for privileged endpoints.
- CORS origin allowlists must be explicit for non-public APIs.
- Prefer security headers (`X-Content-Type-Options`, `X-Frame-Options`/`frame-ancestors`, `Referrer-Policy`, CSP where feasible).

### Secrets and Crypto
- API keys/tokens/secrets MUST be stored in secret store or environment variables; never in source.
- Passwords MUST be hashed with `password_hash`/`password_verify`; no plaintext or reversible storage.
- Never log secrets, full tokens, or sensitive personal data.

### Error Handling and Logging
- Return safe, structured error messages to clients; keep sensitive stack details in server logs only.
- Log auth failures, access denials, validation failures, and destructive admin actions with actor + timestamp.
- Silent catch blocks are prohibited except explicitly non-critical paths with clear rationale.

### Change Validation
- For endpoint/security changes, run: PHP lint, TypeScript check, and repository hygiene before completion.
- Security-impacting migrations must include rollback notes and verification steps.
...
### 🛡️ Data Consistency & Type Safety: The "Shared Types" Protocol
...
// ❌ WRONG: Fallback that masks failures
try {
...
// ✅ CORRECT: Explicit error handling with notification
try {
    $categories = Database::queryAll("SELECT * FROM categories");
} catch (Exception $e) {
...
### 🛡️ Help & Tooltip Mandate (Jan 2026)
...
4. **Tone & Voice:** Tooltips must adhere to the "Whimsical Frog" snarky brand voice—humorous, slightly cynical, explanatory content (~2-3 sentences).
...
| Danger buttons (`.btn-danger`) | red (#ef4444) | darker red (#dc2626) ⚠️ Exception |

#### Example CSS
```css
/* Primary → Secondary on hover */
.btn-primary:hover {
...
1. **Use standard classes** — they inherit from `buttons-hover.css` automatically
2. **If exception needed:** Add to `buttons-hover.css` with comment
3. **Never add redundant hover rules** — add comment: `/* Hover: buttons-hover.css */`
