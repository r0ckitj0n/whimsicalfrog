...
function wf_seed_default_breakdowns(string $sku, float $initialCostPrice = 0.0, float $initialRetailPrice = 0.0): void
{
    if (!preg_match('/^[A-Za-z0-9-]{3,64}$/', $sku)) {
        return;
    }

    $initialCostPrice = max(0.0, round($initialCostPrice, 2));
    $initialRetailPrice = max(0.0, round($initialRetailPrice, 2));
    ...
    if (wf_table_exists('cost_factors')) {
        ...
        if (((int) ($existingCostFactors['c'] ?? 0)) === 0) {
            $defaultCostFactors = [
                ['category' => 'materials', 'label' => 'Manual Materials', 'cost' => $initialCostPrice],
