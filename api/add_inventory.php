...
function wf_seed_default_breakdowns(string $sku): void
{
    ...
    $currentCostPrice = round(max(0, (float) ($itemPricing['cost_price'] ?? 0)), 2);
    $currentRetailPrice = round(max(0, (float) ($itemPricing['retail_price'] ?? 0)), 2);
    ...
    if (wf_table_exists('cost_factors')) {
        ...
        if (((int) ($existingCostFactors['c'] ?? 0)) === 0) {
            $defaultCostFactors = [
                ['category' => 'materials', 'label' => 'Manual Materials', 'cost' => $currentCostPrice],
