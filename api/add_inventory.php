...
function wf_seed_default_breakdowns(string $sku, float $initialCostPrice, float $initialRetailPrice): void
{
    if (!preg_match('/^[A-Za-z0-9-]{3,64}$/', $sku)) {
        return;
    }

    $initialCostPrice = max(0, round($initialCostPrice, 2));
    $initialRetailPrice = max(0, round($initialRetailPrice, 2));
