...
function wf_seed_default_breakdowns(string $sku, float $costPrice, float $retailPrice): void
{
    ...
    $createdCostFactors = false;
    $createdPriceFactors = false;
    $normalizedCostPrice = round(max(0, $costPrice), 2);
    $normalizedRetailPrice = round(max(0, $retailPrice), 2);
    ...
    if (wf_table_exists('cost_factors')) {
        ...
        if (((int) ($existingCostFactors['c'] ?? 0)) === 0) {
            $defaultCostFactors = [
                ['category' => 'materials', 'label' => 'Manual Materials', 'cost' => $normalizedCostPrice],
    ...
    if (wf_table_exists('price_factors')) {
        ...
        if (((int) ($existingPriceFactors['c'] ?? 0)) === 0) {
            Database::execute(
                ...
                [$sku, $normalizedRetailPrice]
            );
            $createdPriceFactors = true;
        }
    }

    if ($createdCostFactors) {
        wf_sync_item_cost_price_from_factors($sku);
    }
    if ($createdPriceFactors) {
