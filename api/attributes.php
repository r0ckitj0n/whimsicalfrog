    ...
    switch ($action) {
        case 'list': {
            ...
            if (table_exists($db, 'global_sizes')) {
                ...
                foreach ($rows as $r) {
                    $val = (string)($r['size_code'] ?: $r['size_name']);
                    $data['size'][] = ['value' => $val];
                }
            }
        ...
        case 'add': {
            ...
            if (!in_array($type, $validTypes, true)) {
                Response::validationError('Invalid type');
            }
            if ($value === '') {
            ...
            if ($type === 'gender' && table_exists($db, 'global_genders')) {
                ...
                $next = 0;
                if ($hasOrder) {
                    $row = Database::queryOne('SELECT COALESCE(MAX(display_order), -1) + 1 AS n FROM global_genders');
                    $next = (int)($row['n'] ?? 0);
                }
                ...
                $next = 0;
                if ($hasOrder) {
                    $row = Database::queryOne('SELECT COALESCE(MAX(display_order), -1) + 1 AS n FROM global_sizes');
                    $next = (int)($row['n'] ?? 0);
                }
                ...
                $next = 0;
                if ($hasOrder) {
                    $row = Database::queryOne('SELECT COALESCE(MAX(display_order), -1) + 1 AS n FROM global_colors');
                    $next = (int)($row['n'] ?? 0);
                }
        ...
        case 'rename': {
            ...
            if ($old === '' || $new === '') {
                Response::validationError('Both old_value and new_value required');
            }
            $db = Database::getInstance();
            if ($type === 'gender' && table_exists($db, 'global_genders')) {
