...
require_once __DIR__ . '/../includes/helpers/ContentToneHelper.php';
...
try {
    ...
    switch ($action) {
        case 'get_all':
            Response::json(['success' => true, 'options' => Database::queryAll("SELECT * FROM content_tone_options ORDER BY display_order, label")]);
            break;
        case 'get_active':
            Response::json(['success' => true, 'options' => Database::queryAll("SELECT * FROM content_tone_options WHERE is_active = 1 ORDER BY display_order, label")]);
            break;
        case 'add':
            ...
            try {
                Database::execute("INSERT INTO content_tone_options (value, label, description, display_order) VALUES (?, ?, ?, ?)", [$v, $l, trim($input['description'] ?? ''), (int)($input['display_order'] ?? 0)]);
        ...
        case 'update':
            ...
            $affected = Database::execute("UPDATE content_tone_options SET value = ?, label = ?, description = ?, is_active = ?, display_order = ? WHERE id = ?", [$v, $l, trim($input['description'] ?? ''), isset($input['is_active']) ? (bool)$input['is_active'] : true, (int)($input['display_order'] ?? 0), $id]);
        ...
        case 'delete':
            ...
            if (Database::execute("DELETE FROM content_tone_options WHERE id = ?", [$id]) > 0) Response::json(['success' => true]); else Response::error('Not found', null, 404);
            break;
        ...
        case 'initialize_defaults':
            Response::json(array_merge(['success' => true], ContentToneHelper::initializeDefaults()));
            break;
        default:
