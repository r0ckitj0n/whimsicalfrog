...
function handleGet(PDO $pdo): void
{
    $action = $_GET['action'] ?? '';
    if ($action === 'catalog') {
        // Scan CSS files and extract class names
        $cssDir = __DIR__ . '/../src/styles';
        $allClasses = [];
        $sources = [];
        ...
        if (is_dir($cssDir)) {
            ...
            foreach ($iterator as $file) {
                if ($file->isFile() && $file->getExtension() === 'css') {
                    ...
                    // Match .classname but not pseudo-classes like :hover, :focus, etc.
                    preg_match_all('/\.([a-zA-Z_][\w-]*)/m', $content, $matches);

                    if (!empty($matches[0])) {
                        $fileClasses = array_unique($matches[0]);
                        sort($fileClasses);

                        $sources[] = [
                            'file' => $relativePath,
                            'classes' => array_values($fileClasses)
                        ];
        ...
        // Deduplicate and sort all classes
        $allClasses = array_unique($allClasses);
        sort($allClasses);

        // Sort sources alphabetically by file name (not path)
...
function handlePost(PDO $pdo): void
{
    ...
    if ($cssProperty === '') {
        $errors['css_property'] = 'Required';
    }
    if ($cssValue === '') {
        $errors['css_value'] = 'Required';
    }
