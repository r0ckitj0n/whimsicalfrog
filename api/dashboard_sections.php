...
// Development bypass: allow access on localhost for dashboard configuration
$isDevelopment = (
...
SQL;
        try {
            $db->exec($sql);
        } catch (Throwable $____) { /* ignore; will fail later if unusable */
        }
    }
}
...
try {
    ...
    try {
        if (class_exists('Logger') && method_exists('Logger', 'exception')) {
