...
echo "WF Dev DB Check\n";

echo "PHP: ".PHP_VERSION."\n";

echo "WHF_ENV: ".(getenv('WHF_ENV') ?: 'not set')."\n";
  ...
  if (class_exists('Database')) {
    ...
    try {
      ...
      if ($stmt && $stmt->fetch()) {
        ...
        echo "backgrounds table: NOT FOUND\n";
      }
    } catch (Throwable $e2) {
    ...
    try {
      ...
      if ($stmt && $stmt->fetch()) {
        $c = $pdo->query('SELECT COUNT(*) AS c FROM items')->fetch();
        echo "items count: ".(int)($c['c'] ?? 0)."\n";
      } else {
        echo "items table: NOT FOUND\n";
      }
    } catch (Throwable $e3) {
      echo "items probe error: ".$e3->getMessage()."\n";
    }
