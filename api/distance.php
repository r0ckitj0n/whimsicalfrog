...
try {
    ...
    // Ensure cache table
    try {
        Database::execute("CREATE TABLE IF NOT EXISTS distance_caches (
        ...
        Logger::info('Distance cache table creation check failed', ['error' => $e->getMessage()]);
    }

    $cacheKey = substr(sha1($fromStr . '|' . $toStr), 0, 96);
    $cached = Database::queryOne("SELECT miles, created_at FROM distance_caches WHERE cache_key = ?", [$cacheKey]);
    if ($cached) {
    ...
    try {
        ...
        Logger::warning('Failed to retrieve ORS API key secret', ['error' => $e->getMessage()]);
    }
    $miles = null;
    $estimated = false;
    ...
    try {
        Database::execute(
            "REPLACE INTO distance_caches (cache_key, from_addr, to_addr, miles) VALUES (?,?,?,?)",
            [$cacheKey, $fromStr, $toStr, $miles]
        );
    } catch (Exception $e) {
        Logger::warning('Failed to save distance cache', ['error' => $e->getMessage(), 'key' => $cacheKey]);
    }

    Response::success(['miles' => round($miles, 2), 'cached' => false, 'estimated' => $estimated]);
} catch (Throwable $e) {
    if (class_exists('Logger')) {
        Logger::exception($e, 'distance error', ['endpoint' => 'distance']);
    }
