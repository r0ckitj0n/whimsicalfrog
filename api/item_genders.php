...
try {
    ...
    switch ($action) {
        case 'add':
            ...
            if (empty($item_sku) || empty($gender)) {
                throw new Exception('Item SKU and gender are required');
            }

            // Check if this gender already exists for this item
            $existing = Database::queryOne("SELECT id FROM item_genders WHERE item_sku = ? AND gender = ?", [$item_sku, $gender]);
        ...
        case 'get_all':
            ...
            $genders = Database::queryAll("SELECT * FROM item_genders WHERE item_sku = ? ORDER BY gender", [$item_sku]);
