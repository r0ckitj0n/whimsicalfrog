...
try {
    ...
    foreach ($rooms as $rn) {
        ...
        $bg = Database::queryOne(
            "SELECT name, image_filename, webp_filename, created_at, updated_at FROM backgrounds WHERE room_number = ? AND is_active = 1 LIMIT 1",
            [$rn]
        );
        if (!$bg) {
            ...
            $bg = Database::queryOne(
                "SELECT name, image_filename, webp_filename, created_at, updated_at FROM backgrounds WHERE room_type = ? AND is_active = 1 LIMIT 1",
                [$rt]
            );
        }
