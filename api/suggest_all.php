 ...
 * independent of whether AI analysis succeeds. Previously this endpoint
 * returned a hard HTTP 400 with a dense technical diagnostic string
 * ("provider=... | model=... | fallback_attempted=..."), which the frontend
 * surfaced as an alarming error toast that reads as "everything is broken"
 * even though the photo upload had already succeeded. Respond 200 instead,
 * with a calm, actionable message and the technical diagnostics tucked into
 * `details` for admins/logs, so the UI can offer manual entry instead of a
 * blocking error.
 */
