...
if ($themeWordsEnabled && $themeModeRequested && !empty($enabledThemeWordFields)) {
    ...
    try {
        ...
        error_log('suggest_marketing.php theme words schema init failed: ' . $e->getMessage());
    }
    $themeInspiration = get_whimsical_inspiration(5);
    $wordList = array_map(fn($item) => $item['text'], $themeInspiration);
    $themeWordTokens = array_values(array_filter(array_map('strval', $wordList)));
    if (!empty($wordList)) {
...
function wf_marketing_text_has_any_token($text, $tokens): bool
{
...
function wf_apply_theme_word_to_marketing_title(string $title, array $themeWords): string
{
    $title = trim($title);
    if ($title === '' || empty($themeWords)) return $title;
    $t0 = trim((string) ($themeWords[0] ?? ''));
    $t1 = trim((string) ($themeWords[1] ?? ''));
    if ($t0 === '') return $title;

    $result = $title;
    if (!wf_marketing_text_has_any_token($result, [$t0])) {
        $result = trim($t0 . ' ' . $result);
    }
    // Keep name to 1-2 theme words; avoid forcing a second token if the title is already long.
    if ($t1 !== '' && strlen($result) <= 56 && !wf_marketing_text_has_any_token($result, [$t1])) {
        $result = trim($result . ' - ' . $t1);
    }
    return $result;
}

function wf_apply_theme_word_to_marketing_description(string $description, array $themeWords): string
{
    $description = trim($description);
    if ($description === '' || empty($themeWords)) return $description;
    ...
    foreach ($tokens as $t) {
        if ($t === '') continue;
        if (!wf_marketing_text_has_any_token($result, [$t])) {
            $missing[] = $t;
        }
        if (count($missing) >= 4) break;
    }
    if (empty($missing)) return $description;
    ...
    $s2 = '';
    if ($t2 !== '' && $t3 !== '') {
        $s2 = "A {$t2} splash of {$t3} whimsy in every sip.";
    } elseif ($t2 !== '') {
        $s2 = "A touch of {$t2} whimsy in every sip.";
    }

    $suffixes = array_values(array_filter([trim($s1), trim($s2)]));
    if (empty($suffixes)) return $description;

    return trim(rtrim($result, ". \t\n\r\0\x0B") . '. ' . implode(' ', $suffixes));
}

function wf_marketing_collect_used_inspiration(array $inspiration, string $finalTitle, string $finalDescription): array
{
    if (empty($inspiration)) {
        return [];
    }
    $combined = strtolower(trim($finalTitle . ' ' . $finalDescription));
    if ($combined === '') {
        return [];
    }
    $used = [];
    foreach ($inspiration as $tw) {
        $tok = is_array($tw) ? trim((string) ($tw['text'] ?? '')) : '';
        if ($tok === '') continue;
        if (stripos($combined, strtolower($tok)) !== false) {
            $used[] = $tw;
        }
    }
    return $used;
...
try {
    ...
    // Deterministically apply theme words so downstream "refine item info from marketing"
    // doesn't accidentally wipe out theme words inserted in the info step.
    // This respects AI Settings toggles and only runs when theme mode is requested.
    $themeApplied = false;
    $usedThemeInspiration = [];
    if ($themeWordsEnabled && $themeModeRequested && !empty($themeWordTokens)) {
        $beforeTitle = (string) ($marketingData['title'] ?? '');
        $beforeDescription = (string) ($marketingData['description'] ?? '');

        if (!empty($themeWordFieldSettings['name/title'])) {
            $marketingData['title'] = wf_apply_theme_word_to_marketing_title($beforeTitle, $themeWordTokens);
        }
        if (!empty($themeWordFieldSettings['description'])) {
            $marketingData['description'] = wf_apply_theme_word_to_marketing_description($beforeDescription, $themeWordTokens);
        }

        $themeApplied = ($beforeTitle !== (string) ($marketingData['title'] ?? ''))
            || ($beforeDescription !== (string) ($marketingData['description'] ?? ''));

        // Only log words that actually appear in final outputs (prevents burning caps).
        if ($themeApplied && !empty($themeInspiration)) {
            $usedThemeInspiration = wf_marketing_collect_used_inspiration(
                $themeInspiration,
                (string) ($marketingData['title'] ?? ''),
                (string) ($marketingData['description'] ?? '')
            );
        }
    }
    ...
    if (!empty($sku)) {
        try {
            ...
            if (!empty($usedThemeInspiration) && $themeApplied) {
                log_theme_words_usage($usedThemeInspiration, 'marketing_suggestion', $sku);
            }
    ...
    Response::json([
        ...
        'theme_words_debug' => $debugThemeWords ? [
            ...
            'enabled_fields' => $enabledThemeWordFields,
