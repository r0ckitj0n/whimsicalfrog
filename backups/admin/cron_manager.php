<?php
// Admin Cron Manager - manage and document scheduled maintenance tasks
require_once __DIR__ . '/../includes/functions.php';
require_once __DIR__ . '/../api/config.php';
require_once __DIR__ . '/../includes/auth.php';
require_once __DIR__ . '/../includes/secret_store.php';

// Bootstrap shared layout (header/footer) if not already present
if (!defined('WF_LAYOUT_BOOTSTRAPPED')) {
    $page = 'admin';
    include dirname(__DIR__) . '/partials/header.php';
    if (!function_exists('__wf_admin_cron_footer_shutdown')) {
        function __wf_admin_cron_footer_shutdown() {
            @include __DIR__ . '/../partials/footer.php';
        }
    }
    register_shutdown_function('__wf_admin_cron_footer_shutdown');
}

// Handle token rotation (simple admin-side action)
  ...
  (function() {
    ...
    if (btn) {
      btn.addEventListener('click', async () => {
        const el = btn.previousElementSibling?.getAttribute('href') || '<?php echo addslashes($webCronUrl); ?>';
        try {
