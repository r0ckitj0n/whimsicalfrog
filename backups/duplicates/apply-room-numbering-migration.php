...
try {
    ...
    $canon = function ($rt, $ext) {
        if ($rt === 'landing') {
            return "background_home.$ext";
        }
        if ($rt === 'room_main') {
            return "background_room_main.$ext";
        }
        if (preg_match('/^room(\d+)$/', $rt, $m)) {
            return "background_room{$m[1]}.$ext";
        }
        return null;
    };
    ...
    foreach ($bgRows as $bg) {
        ...
        if ($wantPng && $bg['image_filename'] !== $wantPng) {
            $updates['image_filename'] = $wantPng;
        }
        if ($wantWebp && $bg['webp_filename'] !== $wantWebp) {
            $updates['webp_filename'] = $wantWebp;
        }

        if (!empty($updates)) {
            $set = [];
            foreach ($updates as $col => $val) {
                $set[] = $col . ' = ' . $pdo->quote($val);
            }
            $bgUpdates[] = [
                'id' => (int)$bg['id'],
                'changes' => $updates,
    ...
    $byFinal = [];
    foreach ($projected as $p) {
        if (!isset($byFinal[$p['final_type']])) {
            $byFinal[$p['final_type']] = [];
        }
    ...
    foreach ($byFinal as $type => $rows) {
        ...
        if (!empty($already)) {
            foreach ($already as $r) {
                if (strcasecmp($r['background_name'], 'Original') === 0) {
                    $preferred = $r;
                    break;
                }
            }
    ...
    foreach ($mapRows as $map) {
        ...
        if (preg_match('/^room(\d+)$/', $map['room_type'], $m)) {
            ...
            if ($oldN >= 2 && $oldN <= 6) {
                ...
                if ($map['room_type'] !== $newType) {
                    $updates['room_type'] = $newType;
                }
            }
        }
        if (!empty($updates)) {
    ...
    try {
        ...
        if ($landingUpdate) {
            Database::execute($landingUpdate['sql']);
        }
        ...
        foreach ($roomUpdates as $u) {
            Database::execute($u['sql']);
        }
        ...
        foreach ($bgPreRenameSQL as $sql) {
            Database::execute($sql);
        }
        foreach ($bgDeactivateMovingActiveSQL as $sql) {
            Database::execute($sql);
        }
        foreach ($bgPreDeactivateSQL as $sql) {
            Database::execute($sql);
        }
