...
window.setupImageHeights = function() {
    document.querySelectorAll('[data-height]').forEach(function(el) {
        ...
        if (h) {
            // Use CSS custom property so it passes the inline-style guard
            el.style.setProperty('--wf-height', h);
            el.classList.add('use-data-height');
        }
    });
};
...
var centralFunctions = {
  ...
  hideCustomAlertBox: () => { const box = document.getElementById('customAlertBox'); if (box) box.classList.add('hidden'); },
  closeProductModal: () => closeProductModal(),
  handleFormFocus: (el) => { if (el) el.classList.add('wf-input-focus'); },
  handleFormBlur: (el) => { if (el) el.classList.remove('wf-input-focus'); },
