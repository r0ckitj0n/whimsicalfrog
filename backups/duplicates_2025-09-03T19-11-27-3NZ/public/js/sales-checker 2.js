...
async function checkAndDisplaySalePrice(item, priceElement, unitPriceElement = null, context = 'popup') {
    ...
    try {
        ...
        if (saleData.isOnSale && saleData.discountPercentage) {
            ...
            // Create sale price display using CSS utility classes for size
            const salePriceHTML = `
                <span class="sale-price-original ${finalSizeClasses}">$${originalPrice.toFixed(2)}</span>
                <span class="sale-price-current ${finalSizeClasses}">$${salePrice.toFixed(2)}</span>
                <span class="sale-discount-text ${finalSizeClasses}">(${Math.round(validDiscountPercentage)}% off)</span>
            `;
...
function positionPopupSimple(element, popup) {
    ...
    popup.classList.add('block', 'invisible'); // Hide visually but allow measurement
    const popupRect = popup.getBoundingClientRect();
    const popupWidth = popupRect.width;
    const popupHeight = popupRect.height;
    popup.classList.remove('invisible'); // Make visible again
    ...
    // Apply final positioning using utility classes (no inline styles)
    // Default: place to the right, vertically centered on element
    const placementClasses = [
        'absolute', 'z-50', 'left-full', 'top-1/2', '-translate-y-1/2', 'ml-2'
    ];

    // Remove any previous placement classes we might have added
    popup.classList.remove(
        'fixed','absolute','relative','z-50','left-full','right-full','top-1/2','-translate-y-1/2','ml-2','mr-2','top-2','bottom-2'
    );

    // Ensure the anchor element is relatively positioned
    element.classList.add('relative');

    // Determine horizontal placement without pixel writes
    let horizontal = 'right';
    if (rect.right + popupWidth + margin > viewportWidth) {
        horizontal = 'left';
    }

    // Determine vertical placement tweaks
    let verticalClasses = ['top-1/2', '-translate-y-1/2'];
    if (rect.top - (popupHeight / 2) < margin) {
        verticalClasses = ['top-2'];
    } else if (rect.bottom + (popupHeight / 2) > viewportHeight - margin) {
        verticalClasses = ['bottom-2'];
    }

    const finalClasses = ['absolute','z-50'];
    if (horizontal === 'right') {
        finalClasses.push('left-full','ml-2');
    } else {
        finalClasses.push('right-full','mr-2');
    }
    finalClasses.push(...verticalClasses);
    popup.classList.add(...finalClasses);

    console.log(`Popup placed ${horizontal} with classes: ${finalClasses.join(' ')}, width=${popupWidth}, height=${popupHeight}`);
}
...
function addSaleBadgeToCardWithDiscount(itemCard, discountPercentage) {
    ...
    itemCard.classList.add('relative');
...
// Duplicate keepPopupVisible removed (already defined earlier)
