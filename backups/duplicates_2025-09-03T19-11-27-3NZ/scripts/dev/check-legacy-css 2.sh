...
matches=$(grep_repo '(_deprecated_css_endpoints|css_generator\.php|global_css_rules\.php)' './scripts/dev/check-legacy-css.sh')
if [[ -n "$matches" ]]; then
...
matches=$(grep_repo '\bcustom_css\b' './api/website_config.php' './scripts/dev/check-legacy-css.sh')
if [[ -n "$matches" ]]; then
