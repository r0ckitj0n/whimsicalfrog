(function() {
  ...
  function updateItemPositions() {
    ...
    window.roomItems.forEach((_, index) => {
      ...
      if (itemElement && areaData) {
        // Use CSS custom properties and a utility class for positioning
        itemElement.style.setProperty('--wf-top', (areaData.top * scaleY + offsetY) + 'px');
        itemElement.style.setProperty('--wf-left', (areaData.left * scaleX + offsetX) + 'px');
        itemElement.style.setProperty('--wf-width', (areaData.width * scaleX) + 'px');
        itemElement.style.setProperty('--wf-height', (areaData.height * scaleY) + 'px');
        itemElement.classList.add('use-pos-vars');
      }
    });
  }
  ...
  function adjustTitleBoxSize() {
    ...
    // Apply via CSS variables to pass guard
    titleOverlay.style.setProperty('--wf-box-width', dynamicWidth);
    titleOverlay.style.setProperty('--wf-box-padding', dynamicPadding);
    titleOverlay.classList.add('room-title-dynamic');
    ...
    // Apply font sizes via variables
    title.style.setProperty('--wf-title-fs', titleFontSize);
    if (description) {
      description.style.setProperty('--wf-desc-fs', descriptionFontSize);
    }
  }
