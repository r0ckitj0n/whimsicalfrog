...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        if ($action === 'create') {
            ...
            // Build subscribers query based on campaign target audience
            $targetAudience = strtolower($campaign['target_audience'] ?? 'all');
            switch ($targetAudience) {
                case 'customers':
                    // Send only to subscribers who are also registered customers (non-admin users)
                    $subscribers_sql = "SELECT s.id, s.email
                    ...
                    break;
                case 'non-customers':
                    // Send only to subscribers who are not registered customers
                    $subscribers_sql = "SELECT s.id, s.email
                                        FROM email_subscribers s
                                        ...
                                        WHERE s.status = 'active'
                                          AND u.id IS NULL";
                    break;
                default: // 'all' or any unspecified value
