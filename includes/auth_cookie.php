...
function wf_auth_make_cookie($user_id, int $ttlSeconds = 60 * 60 * 24 * 7): array
{
    ...
    if ($secret === null) {
        throw new RuntimeException('WF_AUTH_SECRET must be configured before issuing persistent auth cookies');
    }
...
function wf_auth_set_cookie($user_id, string $domain, bool $secure): void
{
    try {
        ...
        error_log('[auth_cookie] Persistent auth cookie not issued: ' . $e->getMessage());
