            ...
            if (empty($_SESSION['user'])) {
                ...
                if (is_array($parsed) && !empty($parsed['user_id'])) {
                    ...
                    if ($row && !empty($row['id'])) {
                        ...
                        wf_auth_clear_cookie(self::getCookieDomain(), self::isHttps());
                        wf_auth_clear_client_hint(self::getCookieDomain(), self::isHttps());
                        self::debugLog("reconstructSessionFromCookie: Refused cookie reconstruction without a verified user row for uid: {$uid}");
                    }
                } else {
