...
/**
 * Seed explanatory manual cost/price breakdown rows for newly created items.
 */
...
function wf_seed_default_breakdowns(string $sku, float $costPrice, float $retailPrice): void
{
    ...
    $safeCostPrice = round(max(0.0, $costPrice), 2);
    $safeRetailPrice = round(max(0.0, $retailPrice), 2);
    ...
    if (wf_inventory_breakdown_table_exists('cost_factors')) {
        ...
        if (((int) ($existingCostFactors['c'] ?? 0)) === 0) {
            $defaultCostFactors = [
                ['category' => 'materials', 'label' => 'Manual Materials', 'cost' => $safeCostPrice],
