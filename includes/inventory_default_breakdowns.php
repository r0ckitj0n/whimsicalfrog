...
/**
 * Seed explanatory cost/retail breakdown rows for newly created inventory items.
 *
 * The submitted item prices remain the source of truth during item creation; factor
 * editors can later resync items after a deliberate breakdown edit.
 */
function wf_inventory_default_table_exists(string $tableName): bool
{
...
function wf_seed_default_breakdowns(string $sku, float $costPrice, float $retailPrice): void
{
    ...
    if (wf_inventory_default_table_exists('cost_factors')) {
    ...
    if (wf_inventory_default_table_exists('price_factors')) {
