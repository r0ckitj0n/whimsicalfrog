  ...
  try {
    ...
    function setupController() {
      ...
      if (shipMethodSel) shipMethodSel.addEventListener('change', async () => {
        ...
        initTooltips();
        if (shipMethodSel.value === 'USPS') { pmShowShippingBenefitNote('Free USPS shipping on orders $50+.'); }
        else { pmHideShippingBenefitNote(); }
        ensurePlaceButtonState();
        await updatePricing();
      });
      ...
      if (shipMethodSel && shipMethodSel.value === 'USPS') { pmShowShippingBenefitNote('Free USPS shipping on orders $50+.'); }
      else { pmHideShippingBenefitNote(); }
      checkSquareSettings();
