...
if (php_sapi_name() !== 'cli') { fwrite(STDERR, "Run from CLI only.\n"); exit(2); }

$table = $argv[1] ?? null;
if (!$table) { fwrite(STDERR, "Usage: php scripts/db/copy_table_dev_to_live.php <table>\n"); exit(3); }
...
if (!$exists) { fwrite(STDERR, "Source table not found on DEV: $table\n"); exit(4); }
...
if (!$createSql) { fwrite(STDERR, "SHOW CREATE TABLE returned nothing for $table\n"); exit(5); }
