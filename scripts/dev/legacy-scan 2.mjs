...
const KNOWN_KEEP_FILES = new Set([
  'api/smoke_tests.php',
]);
...
function walk(dir, out) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      if (PATTERNS.some(p => tokens.includes(p))) out.push(rel);
    }
  }
}
...
const unreferenced = candidates.filter(rel => !KNOWN_KEEP_FILES.has(rel) && !isReferenced(rel));
