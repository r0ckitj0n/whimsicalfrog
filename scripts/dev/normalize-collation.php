...
try {
    ...
    $getForeignKeys = function(string $schema) {
        ...
        $rows = Database::queryAll($sql, [$schema]);
        $byConstraint = [];
        foreach ($rows as $r) { $byConstraint[$r['CONSTRAINT_NAME']][] = $r; }
        return $byConstraint;
    };
    ...
    if ($diagnostics) {
        ...
        foreach ($cols as $table => $cmap) {
            foreach ($cmap as $name => $def) {
                ...
                if (!in_array($dt, $textTypes, true)) continue;
        ...
        if ($diffCount === 0) echo "All text columns match target.\n";
        ...
        foreach ($fks as $fkName => $parts) {
            foreach ($parts as $p) {
                ...
                if (!in_array($dtChild, $textTypes, true) || !in_array($dtParent, $textTypes, true)) continue;
                $cMismatch = ($child['CHARSET'] ?? '') !== ($parent['CHARSET'] ?? '')
                             || ($child['COLLATION_NAME'] ?? '') !== ($parent['COLLATION_NAME'] ?? '')
                             || (string)($child['LEN'] ?? '') !== (string)($parent['LEN'] ?? '');
                if ($cMismatch) {
                    $fkDiff++;
                    echo sprintf("FK %s: %s.%s(%s/%s len=%s) -> %s.%s(%s/%s len=%s)\n",
                        $fkName,
                        $t, $tc, $child['CHARSET'] ?? 'NULL', $child['COLLATION_NAME'] ?? 'NULL', $child['LEN'] ?? 'NULL',
                        $rt, $rc, $parent['CHARSET'] ?? 'NULL', $parent['COLLATION_NAME'] ?? 'NULL', $parent['LEN'] ?? 'NULL'
                    );
        ...
        if ($fkDiff === 0) echo "No FK text-column definition mismatches found.\n";
    ...
    echo "> DB: {$sqlAlterDb}\n";
    if (!$dryRun) { Database::execute($sqlAlterDb); }

    // 2) Convert every table
    $tables = [];
    $rows = Database::queryAll('SHOW TABLES');
    foreach ($rows as $row) { $tables[] = array_values($row)[0]; }
    ...
    foreach ($tables as $table) {
        ...
        if (!$dryRun) {
            try { Database::execute($sqlConvert); }
            catch (Throwable $te) { echo "! WARN: {$table} convert failed: " . $te->getMessage() . "\n"; }
            try { Database::execute($sqlDefault); }
            catch (Throwable $te) { echo "! WARN: {$table} default failed: " . $te->getMessage() . "\n"; }
        }
    }
