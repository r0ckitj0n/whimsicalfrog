...
foreach ($requiredShipping as $k => $v) {
    if ($v === null || $v === '') {
        throw new RuntimeException("Missing required setting: {$k}");
    }
    if (!is_numeric($v)) {
        throw new RuntimeException("Invalid numeric setting: {$k}");
    }
}
...
if ($taxShippingVal === null || $taxShippingVal === '') {
    throw new RuntimeException('Missing required setting: tax_shipping');
}
...
if ($settingsEnabled) {
    if ($settingsRateRaw === null || $settingsRateRaw === '') {
        throw new RuntimeException('Missing required setting: tax_rate');
    }
    if (!is_numeric($settingsRateRaw)) {
        throw new RuntimeException('Invalid numeric setting: tax_rate');
    }
}
...
if (!$currency) {
    throw new RuntimeException('Missing required setting: currency_code');
}
