...
function v(string $msg)    : void { global $VERBOSE; if ($VERBOSE) logln($msg); }

try {
    $pdo = Database::getInstance();
    $dbNameRow = Database::queryOne('SELECT DATABASE() AS db');
    $dbName = $dbNameRow ? ($dbNameRow['db'] ?? '') : '';
    if ($dbName === '') throw new RuntimeException('Unable to determine current database name');
    ...
    } catch (Throwable $e) { $bgTableExists = false; }
    ...
    foreach ($cols as $c) {
        ...
        if ($table === 'backgrounds' && in_array($col, ['image_filename', 'webp_filename'], true)) continue;
