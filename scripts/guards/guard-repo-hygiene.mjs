...
async function findDuplicateSuffixedFiles() {
  ...
  const dupRe = /\s(?:[2-9]|[1-9]\d)(?:\.[^/\\]+)?$/; // files ending with " 2", " 3", ...
  const bakRe = /(\.bak(?:[._-]|$)|(?:^|[._ -])bak\d?(?:[-._]|$))/i;
  const offenders = [];
  for (const rel of all) {
    if (rel.startsWith('backups/duplicates/')) continue; // allowed quarantine
    if (rel.startsWith('backups/bak/')) continue; // allowed quarantine
    const base = rel.split(/[\\/]/).pop() ?? '';
    if (dupRe.test(base) || bakRe.test(base)) offenders.push(rel);
  }
  return offenders;
}
...
async function main() {
  ...
  if (problems.length) {
    ...
    console.error(' - Move duplicate-suffixed files into backups/duplicates/ preserving relative paths');
    console.error(' - Move .bak artifacts into backups/bak/ preserving relative paths');
