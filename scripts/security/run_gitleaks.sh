...
has_native_command() {
  gitleaks "$1" --help >/dev/null 2>&1
}

run_native() {
  if [ "$MODE" = "staged" ]; then
    if has_native_command git; then
  ...
  if has_native_command dir; then
...
run_docker() {
  ...
  if [ "$MODE" = "history" ]; then
    ...
    exec docker run --rm -v "$ROOT_DIR:/repo" -w /repo zricethezav/gitleaks:latest \
      git /repo --log-opts="$COMMIT_RANGE" --redact --no-banner --config .gitleaks.toml
  fi
  exec docker run --rm -v "$ROOT_DIR:/repo" -w /repo zricethezav/gitleaks:latest \
