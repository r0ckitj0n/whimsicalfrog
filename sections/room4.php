...
<style>
    ...
    .room-overlay-wrapper { /* New wrapper for aspect ratio and background */
        ...
        background-image: url('images/room4.webp?v=cb2');
    ...
    .no-webp .room-overlay-wrapper {
        background-image: url('images/room4.png?v=cb2');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadRoomCoordinatesFromDatabase() {
        try {
            const response = await fetch('api/get_room_coordinates.php?room_type=room4');
...
async function loadRoomSettings() {
    try {
        const response = await fetch('/api/room_settings.php?action=get_room&room_number=4');
        const data = await response.json();
        ...
        if (data.success && data.room) {
            ...
            console.log('Loaded room settings for room 4:', room.room_name);
        } else {
