...
if (!$inModal) {
  if (!defined('WF_LAYOUT_BOOTSTRAPPED')) {
    ...
    if (!function_exists('__wf_coupons_footer_shutdown')) {
      function __wf_coupons_footer_shutdown(){ @include __DIR__ . '/../../partials/footer.php'; }
    }
    register_shutdown_function('__wf_coupons_footer_shutdown');
  }
}
if ($inModal) {
...
(function boot(){
  function go(){
    try{ if(window.AdminMarketingModule && window.AdminMarketingModule.openCouponsManager){ window.AdminMarketingModule.openCouponsManager(); return true; } }catch(e){}
    return false;
  }
  if(!go()){
