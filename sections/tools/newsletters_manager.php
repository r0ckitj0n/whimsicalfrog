...
if (!$inModal) {
  if (!defined('WF_LAYOUT_BOOTSTRAPPED')) {
    ...
    if (!function_exists('__wf_newsletters_footer_shutdown')) {
      function __wf_newsletters_footer_shutdown(){ @include __DIR__ . '/../../partials/footer.php'; }
    }
    register_shutdown_function('__wf_newsletters_footer_shutdown');
  }
}
?>
...
(function boot(){
  function go(){
    try{ if(window.AdminMarketingModule && window.AdminMarketingModule.openNewslettersManager){ window.AdminMarketingModule.openNewslettersManager(); return true; } }catch(e){}
    return false;
  }
  if(!go()){
