...
import { IGenderTemplate } from '../../../../hooks/admin/useGlobalEntities.js';
import type { IInventoryOptionLink } from '../../../../types/inventoryOptions.js';

interface GenderTemplatesTabProps {
    templates: IGenderTemplate[];
...
export const GenderTemplatesTab: React.FC<GenderTemplatesTabProps> = ({
    templates,
    links = [],
    onAdd,
    onEdit,
    ...
    links.filter(l => l.option_type === 'gender_template').forEach(l => linkById.set(l.option_id, l));
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h4 className="text-md font-bold text-gray-800">Manage Gender Templates</h4>
                <div className="flex gap-2">
                    <button
                        onClick={onAdd}
                        className="admin-action-btn btn-icon--add"
                        data-help-id="attributes-new-gender-template"
                    />
                </div>
            </div>
                        ...
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    data-help-id="attributes-duplicate-gender-template"
                                />
                                <button
                                    onClick={() => onEdit(tpl.id)}
                                    className="admin-action-btn btn-icon--edit"
                                    data-help-id="attributes-edit-gender-template"
                                />
                                <button
                                    onClick={() => onDelete(tpl.id, tpl.template_name)}
                                    className="admin-action-btn btn-icon--delete"
                                    data-help-id="attributes-delete-gender-template"
                                />
                            </div>
                        </div>
                        <div className="text-xs font-mono text-gray-400">
                            {tpl.gender_count || 0} genders defined
                        </div>
                    </div>
                ))}
            </div>
