...
import { ISizeTemplate } from '../../../../hooks/admin/useGlobalEntities.js';
import type { IInventoryOptionLink } from '../../../../types/inventoryOptions.js';

interface SizeTemplatesTabProps {
...
export const SizeTemplatesTab: React.FC<SizeTemplatesTabProps> = ({
    templates,
    links = [],
    onAdd,
    onEdit,
    ...
    links.filter(l => l.option_type === 'size_template').forEach(l => linkById.set(l.option_id, l));
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h4 className="text-md font-bold text-gray-800">Manage Size Templates</h4>
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        data-help-id="attributes-new-size-template"
                    />
                </div>
            </div>
                        ...
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    data-help-id="attributes-duplicate-size-template"
                                />
                                <button
                                    onClick={() => onEdit(tpl.id)}
                                    className="admin-action-btn btn-icon--edit"
                                    data-help-id="attributes-edit-size-template"
                                />
                                <button
                                    onClick={() => onDelete(tpl.id, tpl.template_name)}
                                    className="admin-action-btn btn-icon--delete"
                                    data-help-id="attributes-delete-size-template"
                                />
                            </div>
                        </div>
                        <div className="text-xs font-mono text-gray-400">
                            {tpl.size_count || 0} sizes defined
                        </div>
                    </div>
                ))}
            </div>
