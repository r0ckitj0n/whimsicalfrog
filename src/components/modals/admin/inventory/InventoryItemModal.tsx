...
export const InventoryItemModal: React.FC<InventoryItemModalProps> = ({
    ...
    const [workingImages, setWorkingImages] = useState<IItemImage[]>([]);
    const [lastAutoGeneratedImagePath, setLastAutoGeneratedImagePath] = useState<string>('');
    ...
    useEffect(() => {
        if (!isAdding || !hasUploadedImage || is_busy) return;
        const primaryPath = (workingImages.find(img => img.is_primary)?.image_path || workingImages[0]?.image_path || '').trim();
        if (!primaryPath || primaryPath === lastAutoGeneratedImagePath) return;

        setLastAutoGeneratedImagePath(primaryPath);
        void handleGenerateAll();
    }, [isAdding, hasUploadedImage, is_busy, workingImages, lastAutoGeneratedImagePath, handleGenerateAll]);
            ...
            >
                ...
                <div className="wf-modal-header modal-header px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-white flex items-center justify-between flex-shrink-0">
                    ...
                    <div className="flex items-center justify-end gap-2 min-w-[320px]">
                        {!isReadOnly && (!isAdding || hasUploadedImage) && (
                            <>
                                <button
                                    type="button"
                        ...
                        {showSaveAction && (
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className={`admin-action-btn ${isAdding ? '' : 'dirty-only'} ${isAdding ? 'btn-icon--add' : 'btn-icon--save'} ${(isAdding || isDirty) ? 'is-dirty' : ''} ${isSaving ? 'is-loading' : ''}`}
                ...
                <div className="modal-body wf-admin-modal-body p-0">
                    <div className="p-6 bg-gradient-to-b from-slate-50 to-slate-100/70 space-y-5">
                            ...
                            <div className="bg-white rounded-2xl border border-amber-200 shadow-sm overflow-hidden">
                                <div className="px-4 py-3 bg-amber-50 border-b border-amber-200">
                                    <h3 className="text-xs font-bold text-amber-900 uppercase tracking-widest flex items-center gap-2">
                                        <span>🖼️</span> First Step: Upload Image
                                    </h3>
                                    <p className="text-xs text-amber-800 mt-1">
                                        Upload the first item image. The system will automatically run Generate All and fill this new item.
                                    </p>
                                </div>
                                <div className="p-4">
                                    {!activeSku ? (
                        ...
                        {isAdding && hasUploadedImage && is_busy && (
                            <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
                                <div className="p-4 bg-emerald-50 text-emerald-900 text-sm font-medium flex items-center gap-2">
                                    <span className="wf-emoji-loader text-base">🪄</span>
                                    Generating item details from uploaded image...
                                </div>
                            </div>
                        )}
