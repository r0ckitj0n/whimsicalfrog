...
(async () => {
  try {
    ...
    if (page.startsWith("admin/")) {
      ...
      const isAdminRouterSettings =
        path.includes("/sections/admin_router.php") &&
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (
        sku &&
        window.WF_Cart &&
        typeof window.WF_Cart.removeItem === "function"
      ) {
        try {
          window.WF_Cart.removeItem(sku);
        } catch (_) {}
        try {
          window.WF_Cart.renderCart && window.WF_Cart.renderCart();
        } catch (_) {}
      }
    },
    true,
