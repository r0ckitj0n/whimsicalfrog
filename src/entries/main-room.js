...
async function loadMainRoomCoordinatesFromDatabase() {
  try {
    const res = await fetch('/api/get_room_coordinates.php?room_type=room_main');
...
function boot() {
  ...
  // Load coordinates and apply
  loadMainRoomCoordinatesFromDatabase().then((loaded) => {
    if (!loaded) return;
    setTimeout(applyDoorCoordinates, 100);
    window.addEventListener('load', () => setTimeout(applyDoorCoordinates, 100));
    let t;
    window.addEventListener('resize', () => {
      clearTimeout(t);
      t = setTimeout(applyDoorCoordinates, 100);
    });
  });
}
