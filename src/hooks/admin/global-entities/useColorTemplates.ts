...
export const useColorTemplates = (fetchAll: () => Promise<void>) => {
    ...
    const saveColorTemplate = useCallback(async (data: Partial<IColorTemplate>) => {
        ...
        try {
            ...
            const res = await ApiClient.post<{ success: boolean; template_id?: number; message?: string }>(`/api/color_templates.php?action=${action}&admin_token=${AUTH.ADMIN_TOKEN}`, data);
            if (res?.success) {
                await fetchAll();
                return res;
            }
