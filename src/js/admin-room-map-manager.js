// Admin: Room Map Manager UI logic (Vite module)
...
(function(){
  ...
  async function fetchJSON(url, opts){
    try {
      const res = await ApiClient.request(url, opts || {});
      if (typeof res === 'string') {
        return { res: null, data: null, text: res };
      }
      return { res: null, data: res, text: JSON.stringify(res) };
    } catch (e) {
  ...
  async function createMap(){
    ...
    else { setMsg('rmMessage','Failed to save map'+(data&&data.message?': '+data.message:'')+(text&&!data? ' (Raw: '+String(text).slice(0,120)+')':''),'error'); }
  }
  ...
  async function applyMap(id){
    ...
    else { setMsg('rmMessage','Failed to apply map'+(data&&data.message?': '+data.message:'')+(text&&!data? ' (Raw: '+String(text).slice(0,120)+')':''),'error'); }
  }
  ...
  async function deleteMap(id){
    ...
    else { setMsg('rmMessage','Failed to delete map'+(data&&data.message?': '+data.message:'')+(text&&!data? ' (Raw: '+String(text).slice(0,120)+')':''),'error'); }
  }
