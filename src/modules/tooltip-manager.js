...
function mapSettingsSelectors(elementId) {
  ...
  const pairs = [
    ...
    ['roomsBtn', 'a[href*="/admin/room_main"], [data-action="open-room-settings"]'],
    ...
    ['help-hints-btn', '[data-action="help-toggle-global-tooltips"]'],
    ...
    ['accountSettingsBtn', '[data-action="open-account-settings"], a[href="/admin/account_settings"]'],
    ['secretsManagerBtn', '[data-action="open-secrets-modal"], a[href="/admin/secrets"]'],
    ['costBreakdownBtn', 'a[href="/admin/cost_breakdown_manager"]'],
    ['userManagerBtn', 'a[href="/admin/customers"]'],
...
async function loadTooltips() {
  ...
  try {
    const url = `${API_URL}?action=get&page_context=${encodeURIComponent(page_context)}`;
    const res = await fetch(url, { credentials: 'same-origin' });
    const json = await res.json();
    if (!json?.success) return;
    const tooltips = json.tooltips || [];
...
try {
  const isAdmin = document.body?.dataset?.page?.startsWith('admin') || /^\/admin(\/|$)/i.test(location.pathname);
  if (isAdmin) {
