(function() {
    // This function initializes the room helper script's data from the global window object.
    function initializeRoomHelper() {
        if (!window.roomData) {
            // Data not available, do not proceed.
            return;
        }

        // Assign data from the global object to window variables
        window.roomItems = window.roomData.roomItems || [];
        window.roomNumber = window.roomData.roomNumber || '';
        window.roomType = window.roomData.roomType || '';
        window.ROOM_TYPE = window.roomType; // Legacy compatibility
        window.originalImageWidth = 1280;
        window.originalImageHeight = 896;
        window.baseAreas = window.roomData.baseAreas || [];
        console.log('⚙️ room-helper.js: Initializing with data:', window.roomData);

        if (window.baseAreas && window.baseAreas.length > 0) {
            console.log('✅ room-helper.js: Successfully loaded ' + window.baseAreas.length + ' coordinate sets.');
        } else {
            console.warn('⚠️ room-helper.js: Warning: `baseAreas` is empty or missing. Item positioning will be disabled.');
        }

        // Set up DOM-dependent parts after initialization
        setupDOMListeners();
    }

    // This function sets up listeners that depend on the DOM being ready.
    ...
    // Delegated click handler for modal interactions
    ...
    // Custom event listener for framework/app readiness
    ...
    // Self-initialize the helper
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeRoomHelper);
    } else {
        initializeRoomHelper();
    }
