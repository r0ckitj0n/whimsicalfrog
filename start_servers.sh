...
if [ -n "$MONITOR_PID" ]; then
  ...
  ./server_monitor.sh daemon > monitor.log 2>&1 &
  DAEMON_PID=$!
  sleep 2
...
echo -e "  ./server_monitor.sh stop && pkill -f 'server_monitor.sh daemon'"
echo -e "\n${YELLOW}To restart the servers, run:${NC}"
echo -e "  ./server_monitor.sh restart"
echo -e "\n${YELLOW}To check monitor status, run:${NC}"
echo -e "  ps aux | grep 'server_monitor.sh daemon' | grep -v grep"
