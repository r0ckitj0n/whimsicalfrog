...
#
# Usage:
#   scripts/dev/sync_to_github.sh --repo https://github.com/<user>/<repo>.git [--branch main] [--remote origin] [--update-remote] [--no-verify]
#   scripts/dev/sync_to_github.sh --repo <user>/<repo> --create-repo [--private|--public] [--branch main] [--remote origin] [--no-verify]
#
# Notes:
...
if [[ -z "$REPO" ]]; then
  echo "Error: --repo <git@github.com:USER/REPO.git|https://github.com/USER/REPO.git> is required" >&2
  exit 1
fi
